...
exports.getActiveLoad = catchAsync(async function (req, res, next) {
	const loads = await Load.find({ assigned_to: req.credentials.userID });
...
exports.updateLoad = catchAsync(async function (req, res, next) {
	...
	Load.findOneAndUpdate(
		...
		(err, load) => {
			...
			if (load.status !== 'NEW')
				return next(
					new AppError('You can update only loads with status "NEW".')
				);
...
exports.deleteLoad = catchAsync(async function (req, res, next) {
	...
	Load.findOneAndDelete({ _id: loadID }, (err, load) => {
		...
		if (load.status !== 'NEW')
			return next(
				new AppError('You can delete only loads this status "NEW".', 403)
			);
...
exports.getLoadShippingInfo = catchAsync(async function (req, res, next) {
	const loadID = req.params.id;
	const { userID } = req.credentials;
